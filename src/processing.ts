...
const breakpointSmallerThan = (breakpoint: number, width: number, height: number) => {
...
export const generateOptimizedImages = async (image: string, breakpoints: number[], sizeOptimization: IOptions['sizeOptimization']) => {
    console.log(image);

    const { width = 0, height = 0, format } = await getMetadata(image);
    // @ts-ignore
    const buffer = await sharp(image).jpeg({ quality: sizeOptimization }).toBuffer();
