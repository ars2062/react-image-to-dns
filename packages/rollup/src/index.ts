...
export default function image(options: IOptions = {}): Plugin {
  ...
  const images: string[] = [];
  return {
    name: "react-image-to-dns-rollup",
    async transform(_, id) {
      ...
      const componentSrc = await import("./component.template").then(
        (res) => res.default
      );
      let component = "";
      if (
        process.env.NODE_ENV === "production" &&
        FORMATS_TO_OPTIMIZE.includes(ext as keyof FormatEnum)
      ) {
        const FILE_URL = `${cdnPath}/${name}-${
          breakpoints[breakpoints.length - 1]
        }.${ext}`;
        const SIZES = `${breakpoints
          .map((breakpoint) => `(min-width: ${breakpoint}px) ${breakpoint}w`)
          .join(", ")}`;
        const SOURCE_SET = `${breakpoints
          .map(
            (breakpoint) =>
              `${cdnPath}/${name}-${breakpoint}.${ext} ${breakpoint}w`
          )
          .join(", ")}`;
    ...
    async generateBundle(buildOptions) {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (FORMATS_TO_OPTIMIZE.includes(format)) {
          const newImages = await generateOptimizedImages(
            image,
            breakpoints,
            sizeOptimization
          );
        ...
        } else copyFileSync(image, join(dir, basename(image)));
      }
